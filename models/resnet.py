...
def modify_resnet_model(model):
    ...
    are initialized to the mean weight of RGB channels. All weights are scaled to (3/C)**0.5 
...
def register_model_with_hook(model):    
    """
    Registers a forward hook to capture the outputs of the last conv layer.

    Args:
        model (torch.nn.Module): The model to register the hook with.

    Returns:
        function: A function to get the captured feature maps.
    """
    feature_maps = None
